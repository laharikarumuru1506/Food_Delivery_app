const menu = [
  ...
  },
  {
    ...
    category: "shakes",
    price: 6.99,
    img: "https://images.pexels.com/photos/6413393/pexels-photo-6413393.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
...
let categoryBtns=categories.map(function(category){
  return `<button class="btn filter-btn mx-2" type="button" data-id=${category}>
      ${category}
      </button>`;
});
...
function showMenuItems(menuItem){
  let displayMenu=menuItem.map(function(item){
    return `<div class="card section-item border-0">
       ...
       <button type="button" class="menu-link btn btn-block btn-primary" onClick = "window.location.href='paymentpage.html'">${value}</button>
       </div>`;
  });
